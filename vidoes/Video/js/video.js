...
//	}
	...
	function playAndPause() {
		if(video[0].paused || video[0].ended) {
			video[0].play();
			$('.psVideo-play-one').addClass('psVideo-play-two');
			$('.psVideo-play-btn').removeClass('psVideo-stop').addClass('psVideo-play');
			playSpeed(videoSpeed);
			clearTimeout(psVideoTime); 
			psVideoTime = setTimeout(function(){
				showControl(false);
				$('.psVideo-play-one').hide();
				psVideoI=false;
			},2500)
			...
			video[0].pause();
			$('.psVideo-play-one').removeClass('psVideo-play-two');
			clearTimeout(psVideoTime); 
			psVideoI=false;
	...
	function outFull() {
		$('.psVideo').removeClass('full_screen');
		clearTimeout(timer);
...
//		qiehuan();
	...
	var psVideoI = true,psVideoTime=false;
	$('.psVideo-shade').click(function(e){
		var event = e || event;
		event.stopPropagation();
	})
	$('.psVideo').click(function() {
		if (psVideoI) {
			showControl(false);
			$('.psVideo-play-one').hide();
			psVideoI=false;
...
//		showControl(false);
	...
	$('.psVideo-play-one').on('click', function(e) {
		var event = e || event;
		event.stopPropagation(); 
		playAndPause();
	});
	...
	$('#soundBtn').on('click', soundAndMute);
			...
			$('.clarity_btns').on('click','.clarity',function(){
				$('.clarity').removeClass('moren');
				$(this).addClass('moren');
				var txt = $(this).text();
				$('.clarity_active').text(txt);
			})
